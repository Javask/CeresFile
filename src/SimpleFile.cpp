...
SimpleFile::SimpleFile(const std::filesystem::path& path) : filePath(path) {}

std::optional<std::ofstream> SimpleFile::openWrite(FileWriteMode mode,
                                                   int64_t offset) {
...
std::optional<std::ifstream> SimpleFile::openRead(FileOpenPos pos,
                                                  int64_t offset) {
  if (!fs::exists(filePath)) return {};
...
std::optional<std::vector<char>> SimpleFile::read() {
...
bool SimpleFile::overwrite(std::vector<char> data) {
...
bool SimpleFile::write(std::vector<char> data, int64_t offset) {
...
bool SimpleFile::append(std::vector<char> data) {
...
bool SimpleFile::create(bool overwrite) {
...
void SimpleFile::deleteFile() { fs::remove(filePath); }

bool SimpleFile::exists() { return fs::exists(filePath); }

std::filesystem::file_time_type SimpleFile::lastModified() {
  return fs::last_write_time(filePath);
}

size_t SimpleFile::hash() { return fs::hash_value(filePath); }

std::filesystem::path SimpleFile::getPath() { return filePath; }
